...
Awesome! This means that our API service is ready to serve requests at <a href="http://localhost:3000/" target="_blank">`localhost:3000`</a>.

If you visit <a href="http://localhost:3000/" target="_blank">`localhost:3000`</a> in your browser, you'll be sending a `GET` request to the root API endpoint. You'll notice that the browser is unable to load anything. This is because there is no route handler definition for our root API endpoint.
...
If you visit <a href="http://localhost:3000/" target="_blank">`localhost:3000`</a> in your browser this time, you'll see the JSON response.

<img src={startApiRootResponse} width="100%" style={{ "maxWidth": 500 + "px" }} alt="root endpoint JSON response in the browser" />
...
```js title=noteHandlers.js highlight=5-10
exports.getNote = ( req, res ) => {
  // {id} value from the URI is stored in `req.params.id`.
  const note = notesCtrl.getNoteById( req.params.id );
...
```js title=noteHandlers.js highlight=4
exports.getNote = ( req, res ) => {
  // Use the `res.locals` object to get the note provided by the 
  // `noteValidation` middleware. 
...
```js title=hateoasControllers.js highlight=1
...
```js title=noteValidation.js highlight=30
const usersCtrl = require( "../controllers/usersController" );
const notesCtrl = require( "../controllers/notesController" );
...
```js title=noteHandlers.js highlight=11
exports.createNote = ( req, res ) => {
  // ExpressJS extracts the note's `text` from the request body and stores 
  // it in `req.body`. If no `text` is provided, return `400 Bad Request`.
  if( !( "text" in req.body ) ) {
...
```js title=noteHandlers.js highlight=5
exports.deleteNote = ( req, res ) => {
  // delete the note from the database
  // The auth middleware stores the authenticated user info in `res.locals.user`. 
...
```js title=noteHandlers.js highlight=7-10,13-18,25-27,30
exports.getNotes = ( req, res ) => {
  // get all info about the user
  const user = getUserById( res.locals.user.sub );
...
```js title=hateoasController.js highlight=2,14,26-37,40-51
...
<a href="https://api.slack.com/methods/search.messages" target="_blank">Slack's Search Messages API</a> uses a query-string param called <code>query</code> for generic searching through messages.
...
```js title=noteHandlers.js highlight=8,25
exports.getNotes = ( req, res ) => {
  // get all info about the user
  const user = getUserById( res.locals.user.sub );
...
```js title=hateoasControllers.js highlight=2,14,29,34,43,48
function hateoasifyNotes( responseNotes, params ) {
...
<a href="https://api.slack.com/methods/search.messages" target="_blank">Slack's Search Messages API</a> uses two query-string params called <code>sort</code> and <code>sort_dir</code>. The former specifies which field to perform the sort on and the latter specifies the direction i.e. whether ascending or descending.
...
### Github
<a href="https://docs.github.com/en/rest/search#example" target="_blank">Github's Search API</a> uses the same simple approach to sorting as Slack using the <code>sort</code> and <code>order</code> query-string parameters.
...
### Wix
<a href="https://dev.wix.com/api/rest/contacts/contacts/sorting,-filtering,-and-searching#contacts_contacts_sorting,-filtering,-and-searching_sorting-list-endpoints" target="_blank">Wix's API</a> allows sorting <i>List endpoints</i> using the <code>sort.fieldName</code> and <code>sort.order</code> query-string parameters.
...
```js title=noteHandlers.js highlight=11-12,19-20,38
exports.getNotes = ( req, res ) => {
  // get all info about the user
  const user = getUserById( res.locals.user.sub );
...
```js title=hateoasController.js highlight=2,14,29,34,43,48
function hateoasifyNotes( responseNotes, params ) {
