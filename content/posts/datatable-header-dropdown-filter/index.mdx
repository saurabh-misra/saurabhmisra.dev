---
title: Add a dropdown filter to the table header using DataTable jQuery plugin
date: "2021-02-07"
slug: "/add-dropdown-filter-header-datatable-jquery"
tags:
  - "jQuery"
  - "DataTable"
  - "Quick Tip"
...
The setup before the customization is pretty basic with the table and the usual DataTable initialization script. <a href="https://jsfiddle.net/saurabhmisra87/nL7j4p08/2/" target="_blank">Click here</a> for a full working example. 

```js
$("table.my-table").DataTable({});
```

Now to replace one of the headers with a `<select>` that will act as filter, we need to add the below script to the initialization statement. I have added comments to explain what each line is doing. <a href="https://jsfiddle.net/saurabhmisra87/pgL1yv95/16/" target="_blank">Click here</a> for a full working example.
